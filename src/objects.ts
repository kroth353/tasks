...
export function toShortForm(question: Question): string {
    const idAsString = question.id.toString();
    const firstNineChars = question.name.substring(0, 10);
    return idAsString + ": " + firstNineChars;
}
...
export function toMarkdown(question: Question): string {
    ...
    returnString += question.body;
    if (question.type == "multiple_choice_question") {
        returnString += "\n";
        returnString += question.options
            .map((formatting: string): string => "- " + formatting)
            .join("\n");
    }
    return returnString;
}
